...
def _clean_terms(terms: list[str],
    ...
    datetime_pattern = re.compile(r'^\d{1,2}/\d{1,2}/\d{4}|^\d{4}-\d{2}-\d{2}')
...
def page_map_and_grade(client: BackendClient):
    ...
    if input_mode.startswith("📄"):
        ...
        if uploaded:
            raw_terms = _parse_upload(uploaded)
            # Use cleaned terms if available, otherwise use raw
            terms = st.session_state.get("cleaned_terms") if st.session_state.get("use_cleaned") else raw_terms
            st.info(f"Parsed **{len(raw_terms)}** unique terms from `{uploaded.name}`")
            if "cleaned_terms" in st.session_state:
                st.success(f"✅ Using cleaned data: **{len(terms)}** terms")
            # ── Step 0: Clean Data ──
            with st.expander("🧹 Clean Data (recommended before mapping)", expanded=True):
                ...
                if "clean_stats" in st.session_state:
                    ...
                    preview_df = pd.DataFrame(
                        st.session_state.get("cleaned_terms", [])[:20],
                        columns=["Term"]
                    )
                    ...
                    if st.button("✅ Use Cleaned Data", key="apply_clean"):
                        st.session_state["use_cleaned"] = True
                        st.rerun()
                    if st.session_state.get("use_cleaned"):
                        if st.button("↩️ Reset to raw data", key="reset_clean"):
                            st.session_state.pop("use_cleaned", None)
                            st.session_state.pop("cleaned_terms", None)
                            st.rerun()
            if terms:
